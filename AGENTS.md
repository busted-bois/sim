...
- **CI:** `.github/workflows/ruff.yml` runs `uvx ruff check .` on PRs to main; `.github/workflows/tests.yml` runs `unittest`, `uv run verify-sim-physics-metadata`, `uv run verify-sim-gate-reference`, and `scripts/smoke_attitude_integration.py`. Both enable `setup-uv` dependency caching. Optional: `.github/workflows/extract-simulator-specs.yml` (`workflow_dispatch`) runs extract then both verify commands when `UE_PROJECT_PATH` is set on a Windows runner.
...
- `src/sim_launch.py` — Launcher. Entry point for `uv run sim`, `uv run sim-very-soft`, `uv run sim-low-end`, `uv run calibrate`, `uv run sim-attitude-smoke`, `uv run sim-timesync-smoke`, `uv run sim-highres-imu-smoke`.
...
Helpers live on `PymavlinkFlightClient` and on the `FlightClient` protocol (`submitVelocityLocalNed`, `streamSetPositionTargetLocalNedAsync`, etc.); see `README.md` (MAVLink local setpoint API). Checklist: `docs/set_position_target_pr_test_plan.md`.
