...
def _mat_vec3(
    matrix: tuple[tuple[float, ...], ...], vec: NedVector3
) -> NedVector3:
    return NedVector3(
...
class NedEnvironmentMap:
    ...
    def snapshot(self) -> VehicleNedSnapshot:
        ...
        return VehicleNedSnapshot(
            ...
            spawn_relative_xy=self.spawn_relative_xy(),
        )
    ...
    def from_multirotor_state(
        ...
        *,
        ...
        mapper.update_local_position_ned(
            x=float(pos.x_val),
            y=float(pos.y_val),
            z=float(pos.z_val),
            vx=float(vel.x_val),
        ...
        if orientation is not None:
            ...
            mapper.update_attitude(roll=0.0, pitch=0.0, yaw=yaw)
