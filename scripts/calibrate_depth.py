...
def main():
    ...
    try:
        while collected < samples_to_collect:
            ...
            if rel_map is not None:
                ...
                if np.any(final_mask):
                    ...
                    msg = (
                        f"\r[calibrate] Progress: {collected}/{samples_to_collect} "
                        "samples collected..."
                    )
                    ...
                    msg = (
                        f"\r[calibrate] No valid pixels in frame "
                        f"(Min Dist: {min_gt:.1f}m, Max: {max_gt:.1f}m)    "
                    )
                    sys.stdout.write(msg)
                    sys.stdout.flush()
        ...
        if r_squared > 0.9:
            print("  Excellent: Depth estimation is highly linear and accurate.")
        elif r_squared > 0.7:
            print("  Good: Usable for obstacle avoidance.")
        else:
            print("  Poor: Try collecting data with more variety in distances.")
        print("="*50)
