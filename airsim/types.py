...
class _ImageType(type):
    ...
    def __getattr__(self, key):
        if key == "DepthPlanner":
            print(
                "\033[31m"
                + "DepthPlanner has been (correctly) renamed to DepthPlanar."
                + " Please use ImageType.DepthPlanar instead."
                + "\033[0m"
            )
...
class Vector3r(MsgpackMixin):
    ...
    def __truediv__(self, other):
        if (
            type(other)
            in [int, float] + np.sctypes["int"] + np.sctypes["uint"] + np.sctypes["float"]
        ):
    ...
    def dot(self, other):
        if isinstance(other, type(self)):
            ...
            raise TypeError(
                f"unsupported operand type(s) for 'dot': {str(type(self))} and {str(type(other))}"
            )
    ...
    def cross(self, other):
        if isinstance(other, type(self)):
            ...
            raise TypeError(
                f"unsupported operand type(s) for 'cross': {str(type(self))} and {str(type(other))}"
            )
...
class Quaternionr(MsgpackMixin):
    ...
    attribute_order = [("w_val", float), ("x_val", float), ("y_val", float), ("z_val", float)]
    ...
    def outer_product(self, other):
        if isinstance(other, type(self)):
            ...
            raise TypeError(
                f"unsupported operand type(s) for 'outer_product':"
    ...
    def rotate(self, other):
        if isinstance(other, type(self)):
            ...
            raise TypeError(
                f"unsupported operand type(s) for 'rotate':"
                f" {str(type(self))} and {str(type(other))}"
            )
...
class GpsData(MsgpackMixin):
    ...
    attribute_order = [("time_stamp", np.uint64), ("gnss", GnssReport), ("is_valid", bool)]
