...
class VehicleClient:
    ...
    def ping(self):
        """
        If connection is established then this call will return true otherwise it will
        be blocked until timeout
    ...
    def isApiControlEnabled(self, vehicle_name=""):
        ...
        successful call to `enableApiControl`, `isApiControlEnabled` should return
        true.
    ...
    def simContinueForFrames(self, frames):
        """
        Continue (or resume if paused) the simulation for the specified number of
        frames, after which the simulation will be paused.
    ...
    def confirmConnection(self):
        """
        Checks state of connection every 1 sec and reports it in Console so user can
        see the progress for connection.
        """
        if self.ping():
            print("Connected!")
        else:
        ...
        if server_ver < server_min_ver:
            ...
            print(
                "AirSim server is of older version and not supported by this "
                "client. Please upgrade!"
            )
        elif client_ver < client_min_ver:
            print(ver_info, file=sys.stderr)
            print(
                "AirSim client is of older version and not supported by this "
    ...
    def simSwapTextures(self, tags, tex_id=0, component_id=0, material_id=0):
        ...
        Args:
            tags (str): string of "," or ", " delimited tags to identify on which
                actors to perform the swap
            tex_id (int, optional): indexes the array of textures assigned to
                each actor undergoing a swap

                                    If out-of-bounds for some object's texture
                                    set, it will be taken modulo the number of
                                    textures that were available
            component_id (int, optional):
            material_id (int, optional):

        Returns:
            list[str]: List of objects which matched the provided tags and had
                the texture swap perfomed
        """
    ...
    def simSetTimeOfDay(
        ...
        Sun's position is computed using the coordinates specified in
        ...
        Args:
            is_enabled (bool): True to enable time-of-day effect, False to reset
                the position to original
            start_datetime (str, optional): Date & Time in %Y-%m-%d %H:%M:%S
                format, e.g. `2018-02-12 15:20:00`
            is_start_datetime_dst (bool, optional): True to adjust for Daylight
                Savings Time
            celestial_clock_speed (float, optional): Run celestial clock faster or
                slower than simulation clock. E.g. Value 100 means for every 1
                second of simulation clock, Sun's position is advanced by 100
                seconds so Sun will move in sky much faster
    ...
    def simGetImage(self, camera_name, image_type, vehicle_name="", external=False):
        ...
        # because this method returns std::vector < uint8>, msgpack decides
        # to encode it as a string unfortunately.
    ...
    def simSetPresetLensSettings(
        ...
        self.client.call(
            "simSetPresetLensSettings", preset_lens_settings, camera_name, vehicle_name, external
        )
    ...
    def simRunConsoleCommand(self, command):
        ...
        Affords access to the countless built-in commands such as "stat unit",
        "stat fps", "open [map]", adjust any config settings, etc. etc.
        Allows the user to create bespoke APIs very easily, by adding a custom
        event to the level blueprint, and then calling the console command
        "ce MyEventName [args]". No recompilation of AirSim needed!
    ...
    def simSetVehiclePose(self, pose, ignore_collision, vehicle_name=""):
        ...
        If you don't want to change position (or orientation) then just set
        components of position (or orientation) to floating point nan values
    ...
    def simSetObjectPose(self, object_name, pose, teleport=True):
        ...
        The specified actor must have Mobility set to movable, otherwise there will
        ...
        Args:
            ...
            teleport (bool, optional): Whether to move the object immediately
    ...
    def simListSceneObjects(self, name_regex=".*"):
        ...
        Default behaviour is to list all objects, regex can be used to return
        smaller list of matching objects or actors
    ...
    def simClearDetectionMeshNames(self, camera_name, image_type, vehicle_name="", external=False):
        ...
        """
        self.client.call(
            "simClearDetectionMeshNames", camera_name, image_type, vehicle_name, external
        )
    ...
    def simPrintLogMessage(self, message, message_param="", severity=0):
        ...
        If message_param is supplied, then it's printed next to the message and
        in that case if this API is called with same message value but different
        message_param again then previous line is overwritten with new line
        (instead of API creating new line on display).

        For example, `simPrintLogMessage("Iteration: ", to_string(i))` keeps
        updating same line on display when API is called with different values
        of i.
        The valid values of severity parameter is 0 to 3 inclusive that
        corresponds to different colors.

        Args:
            message (str): Message to be printed
            message_param (str, optional): Parameter to be printed next to the message
            severity (int, optional): Range 0-3, inclusive, corresponding to the
    ...
    def simGetDistortionParams(self, camera_name, vehicle_name="", external=False):
        ...
        Returns:
            List (float): List of distortion parameter values corresponding to
                K1, K2, K3, P1, P2 respectively.
        """
    ...
    def simSetDistortionParam(
        ...
        """
        self.client.call(
            "simSetDistortionParam", str(camera_name), param_name, value, vehicle_name, external
        )
    ...
    def simGetGroundTruthKinematics(self, vehicle_name=""):
        ...
        The position inside the returned KinematicsState is in the frame of the
    ...
    def simGetGroundTruthEnvironment(self, vehicle_name=""):
        ...
        The position inside the returned EnvironmentState is in the frame of the
        vehicle's starting point
    ...
    def getBarometerData(self, barometer_name="", vehicle_name=""):
        """
        Args:
            barometer_name (str, optional): Name of Barometer to get data from,
    ...
    def getMagnetometerData(self, magnetometer_name="", vehicle_name=""):
        """
        Args:
            magnetometer_name (str, optional): Name of Magnetometer to get data
                from, specified in settings.json
            vehicle_name (str, optional): Name of vehicle to which the sensor corresponds to
    ...
    def getDistanceSensorData(self, distance_sensor_name="", vehicle_name=""):
        """
        Args:
            distance_sensor_name (str, optional): Name of Distance Sensor to get
                data from, specified in settings.json
            vehicle_name (str, optional): Name of vehicle to which the sensor corresponds to
    ...
    def simPlotPoints(
        self, points, color_rgba=[1.0, 0.0, 0.0, 1.0], size=10.0, duration=-1.0, is_persistent=False
    ):
        """
        ...
        Args:
            ...
            is_persistent (bool, optional): If set to True, the desired object
                will be plotted for infinite time.
        """
    ...
    def simPlotLineStrip(
        ...
        Args:
            points (list[Vector3r]): List of 3D locations of line start and end
                points, specified as Vector3r objects
            color_rgba (list, optional): desired RGBA values from 0.0 to 1.0
    ...
    def simPlotLineList(
        ...
        Args:
            points (list[Vector3r]): List of 3D locations of line start and end
                points, specified as Vector3r objects. Must be even
            color_rgba (list, optional): desired RGBA values from 0.0 to 1.0
            thickness (float, optional): Thickness of line
            duration (float, optional): Duration (seconds) to plot for
            is_persistent (bool, optional): If set to True, the desired object
    ...
    def simPlotArrows(
        ...
        """
        Plots a list of arrows in World NED frame, defined from
        points_start[0] to points_end[0], points_start[1] to points_end[1],
        ... , points_start[n-1] to points_end[n-1]

        Args:
            points_start (list[Vector3r]): List of 3D start positions of arrow
                start positions, specified as Vector3r objects
            points_end (list[Vector3r]): List of 3D end positions of arrow
                start positions, specified as Vector3r objects
            color_rgba (list, optional): desired RGBA values from 0.0 to 1.0
            thickness (float, optional): Thickness of line
    ...
    def simPlotStrings(
        self, strings, positions, scale=5, color_rgba=[1.0, 0.0, 0.0, 1.0], duration=-1.0
    ):
        """
        ...
        Args:
            ...
            positions (list[Vector3r]): List of positions where the strings
                should be plotted. Should be in one-to-one correspondence with
                the strings' list
    ...
    def simPlotTransformsWithNames(
        ...
        Args:
            ...
            text_color_rgba (list, optional): desired RGBA values from 0.0 to
                1.0 for the transform name
            duration (float, optional): Duration (seconds) to plot for
        """
        self.client.call(
    ...
    def simAddVehicle(self, vehicle_name, vehicle_type, pose, pawn_path=""):
        ...
        Args:
            ...
            pawn_path (str, optional): Vehicle blueprint path, default empty
...
class MultirotorClient(VehicleClient):
    ...
    def takeoffAsync(self, timeout_sec=20, vehicle_name=""):
        ...
        Returns:
            msgpackrpc.future.Future: future. call .join() to wait for method
                to finish. Example: client.METHOD().join()
        """
    ...
    def moveByAngleThrottleAsync(self, pitch, roll, throttle, yaw_rate, duration, vehicle_name=""):
        logging.warning(
            "moveByAngleThrottleAsync API is deprecated, use "
            "moveByRollPitchYawrateThrottleAsync() API instead"
        )
        return self.client.call_async(
    ...
    def moveByManualAsync(
        ...
        Parameters sets up the constraints on velocity and minimum altitude
        while flying. If RC state is detected to violate these constraints then
        ...
        Args:
            ...
            drivetrain (DrivetrainType): when ForwardOnly, vehicle rotates
                itself so that its front is always facing the direction of travel.
                If MaxDegreeOfFreedom then it doesn't do that
                (crab-like movement)
            yaw_mode (YawMode): Specifies if vehicle should face at given angle
                (is_rate=False) or should be rotating around its axis at given
                rate (is_rate=True)
            vehicle_name (str, optional): Name of the multirotor to send this command to
        Returns:
            msgpackrpc.future.Future: future. call .join() to wait for method
    ...
    def moveByRollPitchYawZAsync(self, roll, pitch, yaw, z, duration, vehicle_name=""):
        ...
        - The body frame follows the Front Left Up (FLU) convention, and
            right-handedness.

        - Frame Convention:
            - X axis is along the **Front** direction of the quadrotor.

            | Clockwise rotation about this axis defines a positive **roll**
            | angle.
            | Hence, rolling with a positive angle is equivalent to
            | translating in the **right** direction, w.r.t. our FLU body
            | frame.

            - Y axis is along the **Left** direction of the quadrotor.

            | Clockwise rotation about this axis defines a positive **pitch**
            | angle.
            | Hence, pitching with a positive angle is equivalent to
            | translating in the **front** direction, w.r.t. our FLU body
            | frame.

            - Z axis is along the **Up** direction.

            | Clockwise rotation about this axis defines a positive **yaw**
            | angle.
            | Hence, yawing with a positive angle is equivalent to rotated
            | towards the **left** direction wrt our FLU body frame. Or in an
            | anticlockwise fashion in the body XY / FL plane.
        ...
        Args:
            ...
            z (float): Desired Z value (in local NED frame of the vehicle)
            duration (float): Desired amount of time (seconds), to send this
                command for
            vehicle_name (str, optional): Name of the multirotor to send this
                command to
    ...
    def moveByRollPitchYawThrottleAsync(
        ...
        - Roll angle, pitch angle, and yaw angle are given in **degrees**
            when using PX4 and in **radians** when using SimpleFlight, in the
            body frame.
        - The body frame follows the Front Left Up (FLU) convention, and
            right-handedness.
    ...
    def setAngleRateControllerGains(
        ...
        - Modifying these gains will have an affect on *ALL* move*() APIs.
            This is because any velocity setpoint is converted to an angle
            level setpoint which is tracked with an angle level controllers.
            That angle level setpoint is itself tracked with and angle rate
            controller.
        - This function should only be called if the default angle rate
    ...
    def setAngleLevelControllerGains(
        ...
        """
        - Sets angle level controller gains (used by any API setting angle
            references - for ex: moveByRollPitchYawZAsync(),
            moveByRollPitchYawThrottleAsync(), etc)
        - Modifying these gains will also affect the behaviour of
            moveByVelocityAsync() API.
            This is because the AirSim flight controller will track velocity
            setpoints by converting them to angle set points.
        - This function should only be called if the default angle level
            control PID gains need to be modified.
        - Passing AngleLevelControllerGains() sets gains to default airsim
            values.
    ...
    def setVelocityControllerGains(self, velocity_gains=VelocityControllerGains(), vehicle_name=""):
        ...
        - This function should only be called if the default velocity control
            PID gains need to be modified.
        - Passing VelocityControllerGains() sets gains to default airsim
            values.

        Args:
            velocity_gains (VelocityControllerGains):
                - Correspond to the world X, Y, Z axes.
                - Pass VelocityControllerGains() to reset gains to default
                    recommended values.
                - Modifying velocity controller gains will have an affect on
                    the behaviour of moveOnSplineAsync() and
                    moveOnSplineVelConstraintsAsync(), as they both use
    ...
    def setPositionControllerGains(self, position_gains=PositionControllerGains(), vehicle_name=""):
        ...
        This function should only be called if the default position control
        PID gains need to be modified.
    ...
    def getMultirotorState(self, vehicle_name=""):
        """
        The position inside the returned MultirotorState is in the frame of
    ...
    def getRotorStates(self, vehicle_name=""):
        """
        Used to obtain the current state of all a multirotor's rotors. The
        state includes the speeds, thrusts and torques for all rotors.
